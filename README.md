...
**2024-01-01 ~ 2026-04-20**, 对比最简基线 vs v5 (8 大非共识因子集成):

| 维度 | 最简基线 | v5 (实跑) |
|---|---|---|
| 股票池 | 10 只大蓝筹 (宁德/茅台/五粮液/平安/招行/比亚迪...) | 200 只中小盘 (流通 30-300 亿) |
| 因子数 | 3 个示例 | **113 个** (含 microstructure / seat_network / intraday / announcements) |
| 年化收益 | 11.03% | **62.69%** |
| 最大回撤 | -8.55% | -16.80% |
| **夏普比率** | **0.79** | **1.86** |
| 超额收益 vs 池内等权 | — | **+24.20%** |
| 信息比 | — | **1.21** (baseline 0.91) |
| 换手率 | 0.45% | 77% (月频) |

> **为何差距这么大**: 小盘 alpha 源 (反转 / 龙虎榜席位 / 失败事件 / 日内犹豫度) 在大蓝筹上基本失效——大蓝筹由 beta 驱动,没有行为性错位可利用. v5 把用户 [非共识数据列表](ADVANCED_TRICKS.md) 里的 8 个维度 (公告时间结构 / 失败事件 / 股东微结构 / 席位网络 / 跌停结构 / 分时犹豫度 / 指数掩护 / 沉寂压缩) 全部实现并接入 pipeline, IC 0.11 / IR 1.21 / 超额回撤仅 -6.71%.
